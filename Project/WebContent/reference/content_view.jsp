...
<body>
	...
	%>
		...
		<tr>
			...
			<c:choose>
				<c:when test="${id eq content_view.bUser}">
					<a href="modify_view.rf?bId=${content_view.bId}">수정</a>&nbsp;&nbsp;
					<a href="list.rf?page=<%=session.getAttribute("cpage") %>">목록보기</a>&nbsp;&nbsp;
					<a href="delete.rf?bId=${content_view.bId}">삭제</a>&nbsp;&nbsp;
				</c:when>
				...
				<c:otherwise>
					<a href ="like.rf?bId=${content_view.bId}">좋아요</a>
					<a href="list.rf?page=<%=session.getAttribute("cpage") %>">목록보기</a>&nbsp;&nbsp;
				</c:otherwise>
			</c:choose>
		...
		<table width="750" cellpadding="0" cellspacing="0"border="1" style="margin-left: auto; margin-right: auto;">
		<thead class="thead"> 
			<tr class = "title">
		...
		<tr>				
			<td><a href ='download.rf?bId=${content_view.bId}'>${content_view.fileName}</a></td>
		</tr>
		</table>	
